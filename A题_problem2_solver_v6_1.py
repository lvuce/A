#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Problem 2 / Scene B solver (V6.1): extend the measured V6 wide search.
...
Known previous-run and optional legacy plans are evaluated first to protect scores.

    python run_all_cases.py A题_problem2_solver_v6_1.py ./results/p2_v6_1 all --cores 5

Keep V8.py and (optionally) V10_final.py beside this file, along with the
contest attachment used by the old solver. For a guaranteed V6 floor, set
HUAWEI_BASELINE_PLAN_DIR to the previous V6 plan directory. Optionally set
...
P5  V5 focused single-OP moves and swaps in the reserved final time. The extra
    ...
    use only the time left when wide search stalls.
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Scene-B multicore solver V6.1")
    ...
    parser.add_argument("--baseline-plan-dir",
                        help="Prior V6 plan directory containing case_001_5core.json. Also HUAWEI_BASELINE_PLAN_DIR.")
    ...
    parser.add_argument("--extra-focus-fraction", type=float, default=1.0,
                        help="Fraction of extra time reserved for wider focused moves (default 1.0; V10 gets any leftover if the search stalls).")
