#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Problem 2 / Scene B solver (V7.1): alternate paired and single-OP moves.

Load the best validated V7 refine-only plans, then alternate a critical-chain
pair move, V6.1's wider single-operation search, core-local ordering, and one
last wide pass. Pair moves change the Pipe bottleneck; the following wide pass
can exploit the new placement. Every accepted candidate is measured with the
official Scene-B evaluator, and saved baseline plans protect prior results.

Set HUAWEI_REFINE_ONLY=1 and HUAWEI_EXTRA_TIME=90 together with
HUAWEI_BASELINE_PLAN_DIR pointing to the V7-refine plan directory. This skips
the full initial portfolio and spends up to 90 seconds on the new sequence.

    python run_all_cases.py A题_problem2_solver_v7_1_hybrid.py ./p2_v7_1_hybrid quick

Keep V8.py beside this file, along with the
contest attachment used by the old solver. For a guaranteed V7 floor, set
HUAWEI_BASELINE_PLAN_DIR to the V7-refine plan directory. Optionally set
...
P5  V5 focused single-OP moves in the standard search, and a four-stage extra
    search: paired moves, wide single-OP moves, targeted ordering, wide moves.
...
class Solver:
    def __init__(self, graph, num_cores, config_path, time_limit=300.0,
                 ...
                 pair_fraction=0.30, refine_only=False):
        self.graph = graph
        ...
        self.pair_fraction = max(0.0, min(1.0, float(pair_fraction)))
        self.refine_only = bool(refine_only)
    ...
    def paired_refine(self, until):
        """Move neighboring critical operations as one placement change.

        A single move can break a useful same-core dependency and introduce a
        new cross-core copy. Paired moves preserve that edge while moving its
        Pipe load. The approximate estimate only ranks candidates: the
        official Scene-B result alone may replace the champion.
        """
        if self.K < 2 or self.best is None:
            return
        model = self.model
        for round_no in range(5):
            if not self.affordable(until):
                return
            champion = self.best
            placement = self.core_of(champion.grouping, champion.group_core)
            official, _, _ = self.official_times(champion.result)
            if len(official) != model.n:
                return
            chain = self.critical_chain(champion.result, limit=80)
            if not chain:
                return
            pipe_load = defaultdict(float)
            for op_id in model.nodes:
                pipe_load[(placement[op_id], model.pipe[op_id])] += model.dur[op_id]

            proposals, seen = [], set()

            def add(kind, changes, edge_index):
                signature = tuple(sorted(changes.items()))
                if signature in seen or all(placement[op] == k for op, k in changes.items()):
                    return
                seen.add(signature)
                proposals.append((edge_index, kind, changes))

            for edge_index, (kind, source, target) in enumerate(chain[:32]):
                if kind == "cross":
                    # Move the target and its next same-core critical OP onto
                    # the source's core, preserving their local dependence.
                    if edge_index and chain[edge_index - 1][1] == target:
                        following = chain[edge_index - 1][2]
                        if placement[following] == placement[target]:
                            add("join_target_pair", {
                                target: placement[source],
                                following: placement[source]}, edge_index)
                    if edge_index + 1 < len(chain) and chain[edge_index + 1][2] == source:
                        preceding = chain[edge_index + 1][1]
                        if placement[preceding] == placement[source]:
                            add("join_source_pair", {
                                preceding: placement[target],
                                source: placement[target]}, edge_index)
                    other_cores = sorted(
                        (k for k in range(self.K) if k not in
                         (placement[source], placement[target])),
                        key=lambda k: (
                            pipe_load[(k, model.pipe[source])] +
                            pipe_load[(k, model.pipe[target])], k))
                    for dest in other_cores[:2]:
                        add("join_third_core", {source: dest, target: dest}, edge_index)
                elif placement[source] == placement[target]:
                    other_cores = sorted(
                        (k for k in range(self.K) if k != placement[source]),
                        key=lambda k: (
                            pipe_load[(k, model.pipe[source])] +
                            pipe_load[(k, model.pipe[target])], k))
                    for dest in other_cores[:2]:
                        add("shift_pair", {source: dest, target: dest}, edge_index)
                if len(proposals) >= (28 if model.n > 5000 else 56):
                    break
            if not proposals:
                return

            reference = self.estimate(placement)[0]
            ranked = []
            for edge_index, kind, changes in proposals:
                if not self.affordable(until):
                    return
                ...
                try:
                    ...
                    continue
                ranked.append((approx, edge_index, kind, changes,
                               self.fine_core(trial), estimated_start))
            if not ranked:
                return
            first = min(ranked, key=lambda item: (item[1], item[0]))
            shortlist = [first] + [item for item in sorted(ranked, key=lambda x: (x[0], x[1]))
                                   if item is not first][:9]
            improved = False
            observed_keys = self.op_keys("otime", official=official)
            for approx, _, kind, changes, fine_core, estimated_start in shortlist:
                if not self.affordable(until):
                    return
                plan = self.realize(self.fine, fine_core, observed_keys)
                before = self.best.objective
                result = self.evaluate(
                    f"{champion.name}+pair_{kind}_r{round_no}",
                    plan, self.fine, fine_core, "otime")
                if self.best.objective < before:
                    self.accepted["pair_" + kind] += 1
                    improved = True
                    break
                if result is not None and approx <= 1.03 * reference and self.affordable(until):
                    retimed = self.realize(
                        self.fine, fine_core, self.op_keys("time", estimated_start))
                    before = self.best.objective
                    self.evaluate(f"{champion.name}+pair_{kind}_retime_r{round_no}",
                                  retimed, self.fine, fine_core, "time")
                    if self.best.objective < before:
                        self.accepted["pair_retime"] += 1
                        improved = True
                        break
            if not improved:
                return

    def critical_order_refine(self, until):
        """Perturb the order of critical OPs without changing their cores."""
        ...
        for round_no in range(3):
            if not self.affordable(until):
                return
            champion = self.best
            placement = self.fine_core(self.core_of(champion.grouping, champion.group_core))
            official, _, _ = self.official_times(champion.result)
            if len(official) != model.n:
                return
            keys = self.op_keys("otime", official=official)
            chain = self.critical_chain(champion.result, limit=48)
            if not chain:
                return
            # Fine-grained realization also tests whether the old coarse
            # subgraphs unnecessarily constrained an otherwise useful order.
            before = self.best.objective
            self.evaluate(f"{champion.name}+order_fine_r{round_no}",
                          self.realize(self.fine, placement, keys),
                          self.fine, placement, "otime")
            if self.best.objective < before:
                self.accepted["order_fine"] += 1
                continue

            candidates, seen = [], set()
            for edge_index, (kind, source, target) in enumerate(chain[:20]):
                suggestions = ([(source, -1), (target, 1)] if kind == "cross"
                               else [(source, -1), (target, -1)])
                for op_id, direction in suggestions:
                    signature = (op_id, direction)
                    if signature not in seen:
                        seen.add(signature)
                        candidates.append((edge_index, op_id, direction))
            improved = False
            for _, op_id, direction in candidates[:18]:
                if not self.affordable(until):
                    return
                shift = max(model.dur[op_id], 0.003 * champion.result["makespan"])
                shifted = dict(keys)
                shifted[op_id] = (official[op_id] + direction * shift,
                                  model.dfs_pos[op_id])
                plan = self.realize(self.fine, placement, shifted)
                before = self.best.objective
                self.evaluate(f"{champion.name}+order_{op_id}_{direction}_r{round_no}",
                              plan, self.fine, placement, "otime")
                if self.best.objective < before:
                    self.accepted["order_shift"] += 1
                    improved = True
                    break
            if not improved:
                return
    ...
    def solve(self, single=None):
        ...
        if self.refine_only:
            if self.best is None:
                raise ValueError("--refine-only requires a validated previous baseline plan")
            log(self.verbose, f"[phase] refining saved plan: {self.best.objective}")
        else:
        ...
        # Alternating neighborhoods matters: pair moves can change the busy
        # Pipe and cross-core edges, allowing the subsequent wide pass to
        # improve a placement that had stalled in V6.1.
        if self.extra_time > 0 and self.best is not None:
            extra_start = time.monotonic()
            extra_deadline = extra_start + self.extra_time
            pair_deadline = extra_start + self.extra_time * self.pair_fraction
            if self.K >= 2 and self.pair_fraction > 0:
                self.paired_refine(min(extra_deadline, pair_deadline))
                log(self.verbose, f"[phase] paired refine done: {self.best.objective}")
            wide_deadline = min(extra_deadline, max(
                pair_deadline, extra_start + self.extra_time * 0.70))
            if self.K >= 2 and self.affordable(wide_deadline):
                self.focused_refine(wide_deadline, broad=True)
                log(self.verbose, f"[phase] wide refine done: {self.best.objective}")
            order_deadline = min(extra_deadline, max(
                wide_deadline, extra_start + self.extra_time * 0.90))
            if self.affordable(order_deadline):
                self.critical_order_refine(order_deadline)
                log(self.verbose, f"[phase] order refine done: {self.best.objective}")
            if self.affordable(extra_deadline):
                self.focused_refine(extra_deadline, broad=True)
            log(self.verbose, f"[phase] final wide refine done: {self.best.objective}")
        ...
        return {
            ...
            "extra_search_strategy": "paired_then_wide_then_order_then_wide",
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Scene-B multicore solver V7.1 hybrid experiment")
    ...
    parser.add_argument("--baseline-plan-dir",
                        help="Prior best plan directory containing case_001_5core.json. Also HUAWEI_BASELINE_PLAN_DIR.")
    ...
    parser.add_argument("--extra-time", type=float,
                        default=float(os.environ.get("HUAWEI_EXTRA_TIME", "30")),
                        help="Seconds for paired, wide, and ordering passes (default 30; also HUAWEI_EXTRA_TIME).")
    parser.add_argument("--refine-only", action="store_true",
                        default=os.environ.get("HUAWEI_REFINE_ONLY", "").lower() in
                        ("1", "true", "yes"),
                        help="Skip initial search and refine a validated previous plan (also HUAWEI_REFINE_ONLY=1).")
    ...
    parser.add_argument("--pair-fraction", type=float, default=0.30,
                        help="Initial fraction of extra time reserved for paired moves (default 0.30).")
...
def main(argv=None):
    ...
    if not 0 <= args.pair_fraction <= 1:
        raise ValueError("--pair-fraction must be between 0 and 1")
    ...
    if baseline_dir:
        ...
        prior_plans.append(("verified_previous_baseline", baseline_plan))
    ...
    if args.refine_only and not prior_plans:
        raise ValueError("--refine-only needs --baseline-plan-dir or HUAWEI_BASELINE_PLAN_DIR")
    solver = Solver(
