#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Problem 2 / Scene B solver (V5): V8 portfolio + focused official refinement.

Put this file next to V8.py (or V8(1).py) and run it with the original
run_all_cases.py interface. The original V8/Scene-B seed, ordering, slack and
critical-chain phases are preserved. Late search adds a bounded set of single
OP migrations and swaps that target bottlenecks observed in the official
timeline. Every accepted plan is re-evaluated by the official Scene-B evaluator.
The baseline is checkpointed to output_plan as soon as an improvement appears.

    python run_all_cases.py A题_problem2_solver_v5.py ./results/p2_v5 all --cores 2 3 4 5

Keep the contest's official code/ and data/config.txt available, as with the
old solver. --refine-fraction 0 reverts to the old phase allocation for A/B
comparison. A positive value reserves that fraction of the original search
budget for focused refinement; equal-runtime improvement is not guaranteed.
...
try:  # Problem-1 structure generators: preserve the strong V8 portfolio.
    if str(ROOT) not in sys.path:
        sys.path.insert(1, str(ROOT))
    try:
        import V8 as _V8  # noqa: E402
    except ImportError:
        v8_file = ROOT / "V8(1).py"
        if not v8_file.exists():
            raise
        spec = importlib.util.spec_from_file_location("V8", v8_file)
        _V8 = importlib.util.module_from_spec(spec)
        sys.modules["V8"] = _V8
        spec.loader.exec_module(_V8)
except Exception as _v8_error:
    _V8 = None
    _v8_import_message = repr(_v8_error)
else:
    _v8_import_message = None
...
def save_plan(path, plan):
    """Only atomically replace a plan after an official evaluation succeeded."""
    if path is None:
        return
    path = Path(path)
...
class Solver:
    def __init__(self, graph, num_cores, config_path, time_limit=300.0,
                 ...
                 output_plan=None, refine_fraction=0.12):
        self.graph = graph
        ...
        for round_no in range(3):
            ...
            for edge_index, (kind, source, target) in enumerate(chain[:20]):
                if kind == "cross":
                    ...
                    for op_id in (target, source):
                        ...
                        # A swap can break a busy Pipe without shifting its
                        # total load as much as an unbalanced one-way move.
            ...
            for approx, edge_index, kind, changes, placement, estimated_start in chosen:
                ...
                result = self.evaluate(
                    f"{champion.name}+focused_{kind}_r{round_no}",
                    plan, self.fine, placement, "otime")
                if self.best.objective < before:
                    self.accepted["focused_" + kind] += 1
                    improved = True
                    break
                if result is not None and approx <= 1.02 * baseline_estimate and self.affordable(until):
                    ...
                    before = self.best.objective
                    self.evaluate(f"{champion.name}+focused_{kind}_retime_r{round_no}",
                                  plan, self.fine, placement, "time")
                    if self.best.objective < before:
                        self.accepted["focused_retime"] += 1
                        improved = True
                        break
            if not improved:
                return
    ...
    def solve(self, single=None):
        ...
        if self.K >= 2:
            ...
            if reserve > 0 and self.affordable(deadline):
                ...
                log(self.verbose, f"[phase] focused refine done: {self.best.objective}")
...
def parse_args(argv=None):
    ...
    parser.add_argument("--refine-fraction", type=float, default=0.12,
                        help="Final fraction of time budget for single-OP/swap refinement; 0 restores old phase allocation.")
    parser.add_argument("--allow-no-v8", action="store_true",
                        help="Proceed without V8 seeds; normally missing V8 is an error to avoid silent score regression.")
...
def main(argv=None):
    ...
    if _V8 is None and not args.allow_no_v8:
        raise FileNotFoundError(
            "V8.py or V8(1).py is required beside this solver for the strong "
            f"candidate portfolio. Original import error: {_v8_import_message}")
